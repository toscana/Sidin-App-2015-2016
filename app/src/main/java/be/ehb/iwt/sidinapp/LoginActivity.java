...
public class LoginActivity extends FragmentActivity implements OnClickListener {
	...
	private boolean mSchoolsDownloaded = false;
	...
	private void downloadData() {
		...
		if (Utilities.hasInternet(this)) {
			...
			new GetSchoolsASyncTask(this).execute(url);
	...
	public void putSchools(List<School> result) {
		if (result != null) {
			Db4oHelper.getInstance(this).storeSchools(result);

			mSchoolsDownloaded = true;
			changeProgressBarIfPossible();
	...
	private void changeProgressBarIfPossible() {
		if (mTeachersDownloaded && mEventsDownloaded && mImagesDownloaded
				&& mSubscriptionsDownloaded &&mSchoolsDownloaded ) {
			mProgressbar.setVisibility(View.INVISIBLE);
			mTextViewLoginLoading.setVisibility(View.INVISIBLE);
