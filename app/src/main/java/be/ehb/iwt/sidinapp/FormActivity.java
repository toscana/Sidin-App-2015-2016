...
public class FormActivity extends FragmentActivity implements TextWatcher,
	...
	private AutoCompleteTextView mSearchStudentField;
	private StudentSearchAdapter mSearchStudentAdapter;
	private SpinnerCityAdapter mSpinnerCityAdapter;
	private ScrollView mScrollView;
	private View mActivityRootView;
	private AutoCompleteTextView mSearchSchoolField;
	private SchoolSearchAdapter mSearchSchoolAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mSearchStudentAdapter = new StudentSearchAdapter(this,
				R.id.autoCompleteTextViewSearchStudent);
		mSearchStudentField.setAdapter(mSearchStudentAdapter);


		mSearchSchoolAdapter = new SchoolSearchAdapter(this,R.id.autoCompleteTextViewSearchSchool);
		mSearchSchoolField.setAdapter(mSearchSchoolAdapter);
		...
		addListenerForStudentSearch();
		addListenerForSchoolSearch();
	...
	private void addListenerForStudentSearch() {
		mSearchStudentField.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				// TODO Auto-generated method stub
				String email = mSearchStudentField.getText().toString();
				if (email.length() >= 3) {
					...
					mSearchStudentAdapter.setList(subList);
	...
	private void addListenerForSchoolSearch() {
		mSearchSchoolField.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				nbCharsSchoolNew = mSearchSchoolField.getText().length();
			...
			public void beforeTextChanged(CharSequence s, int start, int count,
										  int after) {
				// TODO Auto-generated method stub
				nbCharsSchoolOld = nbCharsSchoolNew;
			...
			public void afterTextChanged(Editable s) {
				// TODO Auto-generated method stub
				String name = mSearchSchoolField.getText().toString();
				if (name.length() >= 3 && nbCharsSchoolNew>nbCharsSchoolOld) {
					// looking in database email adresses for match
	...
	protected void fillFormWithSubscription(Subscription subscription) {
		...
		if (!subscription.getZip().equalsIgnoreCase("0")) {
	...
	private void initializeGuiFields() {
		...
		mSearchStudentField = (AutoCompleteTextView) findViewById(R.id.autoCompleteTextViewSearchStudent);
	...
	private void btnSaveData() {
		if (!getInputOk()) {
			...
			String zip = "";
			try {
				zip = mEditTextZip.getText().toString();
			} catch (Exception e) {
			}
			...
			Db4oHelper.getInstance(this).storeSubscription(
					new Subscription(mEditTextFirstName.getText().toString(),
							...
							mCurrentTeacher, mCurrentEvent, true,mSchoolToSave));
